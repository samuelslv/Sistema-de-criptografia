...
def buscarArquivo(diretorio, search_term, file_extension=".enc"):
    """Pesquisa arquivos de texto que contêm o termo de pesquisa no nome do arquivo, dentro do diretório especificado."""
    # Lista para armazenar os caminhos completos dos arquivos de texto que correspondem ao termo de pesquisa
    arquivosEncontrados = []
    ...
    for filename in os.listdir(diretorio):
        if filename.endswith(file_extension) and search_term in filename:
            # Construir o caminho completo do arquivo
            file_path = os.path.join(diretorio, filename)
            arquivosEncontrados.append(file_path)
